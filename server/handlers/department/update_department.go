...
func (d DepartmentHandlers) UpdateDepartmentHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if !ok || userID == uuid.Nil {
		logger.NewErrMessage("Invalid user ID in context",
			zap.String("operation", "UpdateDepartmentHandler"),
	...
	var updatedDepartment directory.Directory
	...
	// 7. Валидация данных
	if strings.TrimSpace(updatedDepartment.Metadata.Title) == "" {
	...
	// 8. Обновление папки департамента
	if err := fsl.Folder.UpdateFolder(departmentId, &updatedDepartment); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			logger.NewWarnMessage("Department folder not found",
				zap.String("operation", "UpdateDepartmentHandler"),
				zap.String("department_id", departmentId.String()),
			)
			http.Error(w, "Department folder not found", http.StatusNotFound)
			return
		}

		logger.NewErrMessage("Failed to update department folder",
			zap.String("operation", "UpdateDepartmentHandler"),
			zap.String("department_id", departmentId.String()),
			zap.Error(err),
		)
